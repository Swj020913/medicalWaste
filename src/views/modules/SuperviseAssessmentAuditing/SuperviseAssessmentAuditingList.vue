  ...
  <a-card :bordered="false">
    ...
    <el-form :model="form" ref="form" :inline="true" label-width="100px">
      <a-row style="margin-bottom:10px">
        ...
        <a-col v-if="isShow" :span="7">
        <el-form-item label="所属城镇">
          <el-select style="width:250px" v-model="form.townId" @change="getOrgList" placeholder="请选择">
    ...
    <el-table :data="tableData" border style="width: 100%">
      ...
      <el-table-column prop="eventCode" label="事件编码"  align="center"> </el-table-column>
      <el-table-column prop="superviseReason" width="400" label="督导原因"  align="center"> </el-table-column>
      <el-table-column prop="superviseState" label="督导状态"  align="center">
        <template slot-scope="scope">
          <el-tag
            style="width:80px;"
            :type="
      ...
      <el-table-column prop="deptId" label="责任科室"  align="center">
        <template slot-scope="scope">
          {{ filtertoDictText(deptIdList, scope.row.deptId, 'departName') }}
        </template>
      </el-table-column>
      <el-table-column prop="assessmentResults" label="是否整改" width="120px" align="center">
        <template slot-scope="scope">
          {{ filterDictText(assessmentResultsLsit, scope.row.assessmentResults) }}
        </template>
      </el-table-column>
      <el-table-column prop="personLiable" width="120px" label="责任人" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" width="100" align="center">
        <template slot-scope="scope">
          <a @click="edit(scope.row)" v-if="scope.row.superviseState == 0 || scope.row.superviseState == 5" type="text"
            >进行整改</a
          >
          <a
            @click="detail(scope.row)"
            v-if="scope.row.superviseState != 0 && scope.row.superviseState != 5"
            type="text"
            >查看详情</a
          >
        </template>
    ...
    <supervise-assessment-auditing-form ref="modalForm" @ok="modalFormOk"></supervise-assessment-auditing-form>
  </a-card>
</template>
...
import SuperviseAssessmentAuditingForm from './modules/SuperviseAssessmentAuditingForm'
...
export default {
  ...
  mixins: [mixinDevice],
  components: {
    SuperviseAssessmentAuditingForm
  },
  data () {
    return {
      description: '督导考核审批页面',
      form: {
        pageNo: 1,
        pageSize: 10
      },
      ...
      getOrgList(that.form.townId == undefined ? '' : that.form.townId).then(res => {
        if (res.success) {
          ...
          if (that.orgList.length > 0 && !that.isShow) {
            that.form.orgId = that.orgList[0].orgCode
            that.getlist()
          }
        } else {
          that.$message.error(res.message)
